...
static int output_property_mk(lua_State* L, struct display* display, RROutput output, Atom prop)
{
    ...
    if(strcmp(atom_name(name), "EDID") == 0) {
        if(type != XA_INTEGER) {
            failwith("EDID property with unexpected type: %ld", type);
        }
        if(format != 8) {
            failwith("EDID property with unexpected format: %d", format);
        }

        struct edid edid;
        edid_parse(&edid, value, nitems);
        edid_push(L, &edid);
